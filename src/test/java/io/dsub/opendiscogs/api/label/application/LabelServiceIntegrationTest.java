package io.dsub.opendiscogs.api.label.application;
...
import io.dsub.opendiscogs.api.label.infrastructure.LabelRepositoryImpl;
import io.dsub.opendiscogs.api.label.query.LabelQuery;
...
public class LabelServiceIntegrationTest extends AbstractDatabaseIntegrationTest {
  ...
  void setUp() {
    ...
    this.labels = IntStream.rangeClosed(1, 10)
        ...
        .toList();
    r2dbcRepository.saveAll(this.labels).blockLast();
    Flux.fromIterable(this.labels)
        .filter(label -> label.getId() % 2 == 0)
        .flatMap(label -> databaseClient.sql("""
                INSERT INTO label_sub_label
                  (created_at, last_modified_at, parent_label_id, sub_label_id)
                VALUES (NOW(), NOW(), 1, :subLabelId)
                """)
            .bind("subLabelId", label.getId())
            .then())
        .blockLast();
  }
    ...
    Flux.range(1, 2)
        .flatMap(i -> template.insert(Release.class)
            .using(TestUtil.getInstanceOf(Release.class)
                .withId((long) i)
                .withMasterId(null)
                .withReleaseDate(LocalDate.of(2022, i, i))
            ...
            databaseClient
                .sql("""
                    INSERT INTO release_item_artist
                      (created_at, last_modified_at, release_item_id, artist_id)
                    VALUES (NOW(), NOW(), $1, $1)
                    """)
                .bind(0, i)
                .then())
            ...
            .sql(
                """
                    INSERT INTO release_item_format
                      (created_at, last_modified_at, release_item_id, hash, description)
                    VALUES (NOW(), NOW(), :i, :i, 'test-description')
                    """)
            .bind("i", i)
            .then())
        .blockLast();
            ...
            .sql(
                """
                    INSERT INTO label_release_item
                      (created_at, last_modified_at, label_id, release_item_id, category_notation)
                    VALUES (NOW(), NOW(), :i, :i, 'test-catno')
                    """)
            .bind("i", i)
            .then())
        .blockLast();
