...
class ReleaseServiceUnitTest {
  ...
  @Captor
  ArgumentCaptor<ReleaseQuery> queryCaptor;
  @Captor
  ...
  void search() {
    ...
    given(releaseRepository.findAllBy(queryCaptor.capture(), pageCaptor.capture()))
    ...
    assertThat(queryCaptor.getValue()).isEqualTo(query.normalized());
