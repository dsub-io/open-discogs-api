...
import static io.dsub.opendiscogs.api.core.util.JooqUtility.getSortFields;
...
public class ArtistRepositoryImpl implements ArtistRepository {
  ...
  public Flux<ArtistReleaseDTO> findReleasesByArtistId(Long id, Pageable pageable) {
    return Flux.from(
            ctx.select(RELEASE_ITEM.ID.as("id"),
                    RELEASE_ITEM.TITLE.as("title"),
                    RELEASE_ITEM.COUNTRY.as("country"),
                    RELEASE_ITEM.DATA_QUALITY.as("data_quality"),
                    when(RELEASE_ITEM.HAS_VALID_YEAR.isTrue(),
                        extract(RELEASE_ITEM.RELEASE_DATE, DatePart.YEAR))
                        .as("released_year"),
                    when(RELEASE_ITEM.HAS_VALID_MONTH.isTrue(),
                        extract(RELEASE_ITEM.RELEASE_DATE, DatePart.MONTH))
                        .as("released_month"),
                    when(RELEASE_ITEM.HAS_VALID_DAY.isTrue(),
                        extract(RELEASE_ITEM.RELEASE_DATE, DatePart.DAY))
                        .as("released_day"),
                    RELEASE_ITEM.LISTED_RELEASE_DATE.as("listed_release_date"),
                    RELEASE_ITEM.IS_MASTER.as("is_master"),
                    RELEASE_ITEM.NOTES.as("notes"),
                    RELEASE_ITEM.STATUS.as("status"),
                    RELEASE_ITEM.MASTER_ID.as("master_id"),
                    field("role", String.class))
                .from(select(field("release_item_id", Long.class),
                    field("string_agg(distinct trim(r.role), ',')", String.class).as("role"))
                    .from(
                        select(field("release_item_id", Long.class),
                            field("'Main'", String.class).as("role"))
                            .from(RELEASE_ITEM_ARTIST)
                            .where(RELEASE_ITEM_ARTIST.ARTIST_ID.eq(id.intValue()))
                            .unionAll(
                                select(field("release_item_id", Long.class),
                                    field("role", String.class))
                                    .from(RELEASE_ITEM_CREDITED_ARTIST)
                                    .where(RELEASE_ITEM_CREDITED_ARTIST.ARTIST_ID.eq(id.intValue())))
                            .asTable("r")
                    )
                    .groupBy(field("release_item_id")))
                .join(RELEASE_ITEM)
                .on(RELEASE_ITEM.ID.eq(field("release_item_id", Integer.class)))
